...
sealed trait Either[+E,+A] {
  def map[B](f: A => B): Either[E, B] =
    this match {
      // Right returns a valid value and f: A => B,
      // so Right(f(a)) makes sense
      case Right(a) => Right(f(a))
      // Left shouldn't hold a valid value but more for error handling
      // also f: A => B, so anyway you cannot apply f to e,
      // just return Left as is
      case Left(e) => Left(e)
    }
  ...
  def flatMap[EE >: E, B](f: A => Either[EE, B]): Either[EE, B] =
    this match {
      case Left(e) => Left(e)
      case Right(a) => f(a)
      //Look at the memtal model of flatMap I described in Option
      // step 1. f generates a sub-Either
      // step 2. flatMap concats sub-Either (there is only one Either though)
      //   and flattens it i
    }
  ...
  Either[EE, C] = for {
    eitherRightValue1 <- this
    eitherRightValue2 <- b
  } yield f(eitherRightValue1,eitherRightValue2)
  /**
   * There are a number of variations on `Option` and `Either`.
   * If we want to accumulate multiple errors,
